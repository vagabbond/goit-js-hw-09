...
const submitForm = (event)=>{
    ...
    let delay1 = Number(refs.inputDelay.value);
    for(let i = 1; i <= refs.inputAmout.value; i += 1){
        console.log(delay1);
        createPromise(i, delay1).then(({ position , delay  })=>(0, _notiflix.Notify).success(`✅ Fulfilled promise ${position} in ${delay}ms`)).catch(({ position , delay  })=>(0, _notiflix.Notify).failure(`❌ Rejected promise ${position} in ${delay}ms`));
        delay1 = Number(refs.inputDelay.value) + Number(refs.inputStep.value) * i;
    }
};
